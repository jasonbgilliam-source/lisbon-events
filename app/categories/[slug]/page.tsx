...
import { buildCatalogIndex, slugify } from "@/lib/categoryMeta";
...
export default function CategoryPage({ params }: { params: { slug: string } }) {
  ...
  // load catalog, locate this category's canonical name, image and embeds
  React.useEffect(() => {
    (async () => {
      try {
  ...
  // fetch category events: default range today -> +180 days
  async function load() {
    ...
    try {
      ...
      const params = new URLSearchParams({
        ...
        category: categoryName || "", // API normalizes against catalog
      });
      ...
      // Sort oldest-first by start time
  ...
  return (
    <main className="max-w-7xl mx-auto p-0">
      ...
      <div className="max-w-7xl mx-auto px-6 py-6">
        {/* dynamic embeds (bloggers / youtube) */}
        {embeds.length > 0 && (
        ...
        {/* events list by date, oldest first; date column on the left */}
        <section>
